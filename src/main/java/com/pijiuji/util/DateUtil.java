...
public class DateUtil {
    ...
    /**
     * 当前时间加多少分钟
     * @param newDateCreated
     * @param codeValidity
     * @return 返回结果
     */
	public static String addTime(String newDateCreated, String codeValidity) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            Date date = sdf.parse(newDateCreated);
            Calendar instance = Calendar.getInstance();
            instance.setTime(date);
            instance.add(Calendar.MINUTE,Integer.parseInt(codeValidity));
	...
	public static Integer getCurrentMonth() {
		Calendar can = Calendar.getInstance();
		return can.get(Calendar.MONTH);
	}

	public static Integer getCurrentDay() {
		Calendar can = Calendar.getInstance();
		return can.get(Calendar.DAY_OF_MONTH);
	}
	...
	public static Integer getHour(String time, String format) throws Exception {
		try {
			...
			Calendar can = Calendar.getInstance();
			can.setTime(date);
			return can.get(Calendar.HOUR_OF_DAY);
		} catch (Exception e) {
			throw e;
		}
	}
	...
	public static Integer getCurrentMinute() {
		Calendar can = Calendar.getInstance();
		return can.get(Calendar.MINUTE);
	}

	public static Integer getCurrentSecond() throws Exception {
		try {
			Calendar can = Calendar.getInstance();
			return can.get(Calendar.SECOND);
		} catch (Exception e) {
			throw e;
		}
	}
	...
	public static boolean isCurrentDay(String time, String format) throws Exception {
		try {
			...
			if (getYear(time, format) - year == 0 && getMonth(time, format) - month == 0
					&& getDay(time, format) - day == 0) {
				return true;
			}
			return false;
		} catch (Exception e) {
	 ...
	 */
	public static String getCurrentTimeS2() {
		Date date = new Date();
		SimpleDateFormat sdf = new SimpleDateFormat();
		sdf.applyPattern("yyyy-MM-dd HH:mm:ss");
		return sdf.format(date);
